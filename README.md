...
`--init-git` を付けた場合、ZIP作成後に書き出しフォルダだけで初回コミット1件の新規Gitを作ります。開発履歴やリモート設定を引き継がず、ZIPにも`.git`は含めません。初回コミットは作者名MidnaUdonと作者のGitHub公開用アドレス`18381369+MidnaAlfort@users.noreply.github.com`を使い、開発環境の個人メールアドレスを引き継ぎません。
